...
instance Show KerberosAuthResult where
  ...
  show NoSuchUser         = "Error: Wrong username"
  show WrongPassword      = "Error: Wrong password"
  show TimeOut            = "Error: kinit respone timeout"
...
interpretError _ errmsg = fromJust . msum $
  ...
  where
    substr --> error = guard (substr `T.isInfixOf` errmsg) >> Just error
